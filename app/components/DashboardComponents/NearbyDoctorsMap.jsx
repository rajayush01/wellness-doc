...
const randomLocationIcon = new L.Icon({
  iconUrl: "https://png.pngtree.com/png-vector/20230413/ourmid/pngtree-3d-location-icon-clipart-in-transparent-background-vector-png-image_6704161.png", // Replace with an actual icon URL for random locations
  iconSize: [25, 35],
  iconAnchor: [12, 35],
});
      ...
      <div className="w-full lg:w-3/5 h-[400px] lg:h-[600px] rounded-lg overflow-hidden shadow-lg">
        {isLoaded && (
          <MapContainer center={userLocation} zoom={13} className="h-full w-full">
            ...
            <Marker position={userLocation} icon={L.icon({ iconUrl: 'https://p7.hiclipart.com/preview/230/805/284/computer-icons-clip-art-location-icon-thumbnail.jpg', iconSize: [25, 35], iconAnchor: [12, 41] })}>
              <Popup>Your Location</Popup>
            </Marker>
            {doctors.map((doctor) => (
              <Marker key={doctor.id} position={[doctor.lat, doctor.lng]} icon={doctorIcon}>
                <Popup style={{ zIndex: 1 }}>{doctor.name}</Popup>
              </Marker>
            ))}
            {randomLocations.map((location) => (
              <Marker key={location.id} position={[location.lat, location.lng]} icon={randomLocationIcon}>
                <Popup style={{ zIndex: 1 }}>Random Location {location.id + 1}</Popup>
              </Marker>
            ))}
          </MapContainer>
        )}
